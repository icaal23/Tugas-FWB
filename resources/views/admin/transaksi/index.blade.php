...
<body class="g-sidenav-show  bg-gray-100">
  ...
  <main class="main-content position-relative max-height-vh-100 h-100 border-radius-lg ">
    ...
    <div class="container-fluid py-2">
      <div class="row">
        <div class="ms-3">
          <h3 class="mb-0 h4 font-weight-bolder">Kelola Transaksi</h3>
          <p class="mb-4">
            Daftar-daftar Transaksi
          </p>
        </div>
            ...
            <tbody>
              ...
              <tr>
                ...
                <td class="align-middle">
                  {{ $transaksi->mobil ? $transaksi->mobil->merk . ' ' . $transaksi->mobil->model : '-' }}
                </td>
                <td class="align-middle">{{ $transaksi->created_at ? $transaksi->created_at->format('d-m-Y') : '-' }}</td>
                <td class="align-middle">Rp.{{ number_format($transaksi->total_harga, 0, ',', '.') }}</td>
                <td class="align-middle">
                  <span class="badge bg-gradient-{{ $transaksi->status == 'selesai' ? 'success' : ($transaksi->status == 'proses' ? 'warning' : 'secondary') }}">
                    {{ ucfirst($transaksi->status) }}
                  </span>
                </td>
                <td class="align-middle text-center">
                  <form action="{{ route('admin.transaksi.destroy', $transaksi->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus transaksi ini?')">
                    @csrf
                    @method('DELETE')
